...
class GateL0RDCell(nn.Module):
    ...
    def forward(self, x_t, h_tminus1=None):
        ...
        if h_tminus1 is None:
            h_tminus1 = torch.zeros((batch_size, self.hidden_size), device=device, dtype=dtype)
        else:
            ...
            if h_tminus1.device != device:
                h_tminus1 = h_tminus1.to(device=device, dtype=dtype)
        ...
        if self.training:
            gate_noise = torch.randn(size=(batch_size, self.hidden_size), device=device, dtype=dtype) * self.gate_noise_level
        else:
            # Gate noise is zero
            gate_noise = torch.zeros((batch_size, self.hidden_size), device=device, dtype=dtype)
