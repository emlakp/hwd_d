...
assert (PROJECT_ROOT / "config").is_dir(), f"Config directory not found: {PROJECT_ROOT}"
...
def load_config():
    """Load and configure Hydra config for RSSM world model training."""
    with initialize_config_dir(version_base="1.3", config_dir=str(PROJECT_ROOT / "config")):
        cfg = compose(config_name="train_wm_debug_rssm")

    # Configure experiment directory and logging
...
def create_dataloader(cfg, train_tfms):
    ...
    batch_size = cfg.world_model.train_batch_size

    def collate_with_vis(batch):
        collated = transpose_collate_wm(batch)
        reset = collated.get("reset")
        if reset is not None:
            collated["reset"] = reset.unsqueeze(-1)
        return {"vis": collated}
...
class DebugDataModule(LightningDataModule):
    """Minimal LightningDataModule to expose transforms to the model during debugging."""
...
def print_data_shapes(sample, batch):
    """Print shapes of sample and batch data for debugging."""
...
def attach_disk_image_logger(model, cfg: OmegaConf) -> None:
    """Override model.log_images to persist debug images to disk."""
    save_root = Path(cfg.exp_dir) / "images"
    save_root.mkdir(parents=True, exist_ok=True)

    def _tensor_to_image(tensor, transform):
        array = transform(tensor.to("cpu").detach().numpy())
    ...
    def log_images_to_disk(
        self,
        gt_img_s: torch.Tensor,
        gt_img_g: torch.Tensor,
        dcd_img_s: torch.Tensor,
        dcd_img_g: torch.Tensor,
        pred_img_s: torch.Tensor,
        pred_img_g: torch.Tensor,
        mode: str,
    ):
        if self.global_step % LOG_IMAGE_EVERY != 0:
            return
        transform = self.trainer.datamodule.train_transforms["out_rgb"]
        step_dir = save_root / mode / f"step_{self.global_step:06d}"
        step_dir.mkdir(parents=True, exist_ok=True)

        def maybe_save(name: str, tensor: torch.Tensor):
            if tensor is None:
                return
            image = _tensor_to_image(tensor, transform)
            image.save(step_dir / f"{name}.png")

        maybe_save("gt_static", gt_img_s)
        maybe_save("dcd_static", dcd_img_s)
        maybe_save("pred_static", pred_img_s)
        maybe_save("gt_gripper", gt_img_g)
        maybe_save("dcd_gripper", dcd_img_g)
        maybe_save("pred_gripper", pred_img_g)

    model.log_images = types.MethodType(log_images_to_disk, model)
...
def main(resume=False, checkpoint_path=None):
    ...
    print_data_shapes(sample, batch_dict["vis"])

    print("\nCreating model...")
    model = create_model(cfg)

    datamodule = DebugDataModule(loader, loader, train_tfms, val_tfms)
    attach_disk_image_logger(model, cfg)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Debug RSSM training script.")
